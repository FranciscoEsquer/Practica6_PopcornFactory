package esquer.francisco.popcornfactory
        ...
        confirm.setOnClickListener {
            ...
            for(clients in peliculas?.get(pos)?.seats!!){
                if(clients.asiento == asiento){
                    Toast.makeText(this,"¡Asiento no disponilbe, ocupado!", Toast.LENGTH_LONG).show()
                    error = true
                }
            }

            if(asiento == -1){
                Toast.makeText(this,"¡Primero debes selecionar al menos 1 asiento!", Toast.LENGTH_LONG).show()
            }else if(!error){
